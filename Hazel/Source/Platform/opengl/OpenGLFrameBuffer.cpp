...
namespace Hazel
{
	namespace Utils
	{
		...
		{
			glCreateTextures(TextureTarget(multisampled), count, outID); //were our textures are created
		}
		...
		static void AttachColorTexture(uint32_t id, int samples, GLenum interlalformat, GLenum format, uint32_t width, uint32_t height, int index)
		{
			bool multisampled = samples > 1;
			if (multisampled)
			{
				glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, samples, interlalformat, width, height, GL_FALSE);
			}
			else
			{
				glTexImage2D(GL_TEXTURE_2D, 0, interlalformat, width, height, 0, format, GL_UNSIGNED_BYTE, nullptr); //allocates memory for textures
			...
			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index, TextureTarget(multisampled), id, 0); //attaches texture to framebuffer at correct index
		}
		...
		static void AttachDepthTexture(uint32_t id, int samples, GLenum format, GLenum attachmentType, uint32_t width, uint32_t height)
		{
			...
			glFramebufferTexture2D(GL_FRAMEBUFFER, attachmentType, TextureTarget(multisampled), id, 0); //attaches texture to framebuffer at correct index
		}
