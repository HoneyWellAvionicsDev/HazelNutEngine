...
namespace Hazel
{
		...
		: m_Path(path)
	{
		...
		stbi_uc* data = stbi_load(path.c_str(), &width, &height, &channels, 0);                        //stores texture on the CPU
		...
		glTextureParameteri(m_RendererID, GL_TEXTURE_MIN_FILTER, GL_NEAREST);                           //texture params such as Min/Magnification and mipmapping
