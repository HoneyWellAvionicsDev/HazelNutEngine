...
#include <filesystem>
	...
	#define INTERNAL_ASSERT_IMPL(type, check, msg, ...) { if(!(check)) { JB##type##ERROR(msg, __VA_ARGS__); DEBUG_BREAK(); } }
	...
	#define JB_ASSERT(...) EXPAND_MACRO( INTERNAL_ASSERT_GET_MACRO(__VA_ARGS__)(_, __VA_ARGS__) )
	#define JB_CORE_ASSERT(...) EXPAND_MACRO( INTERNAL_ASSERT_GET_MACRO(__VA_ARGS__)(_CORE_, __VA_ARGS__) )
#else
	#define JB_ASSERT(...)
	#define JB_CORE_ASSERT(...)
#endif
